...
export default function HorasPage() {
  ...
  async function load() {
    ...
    const { data: employees } = await supabase.from('employees').select('*, stores(name,code)')
      .in('store_id', profile?.store_ids ?? []).eq('active', true).order('name')

    const results = await Promise.all((employees ?? []).map(async emp => {
      const { data: sched } = await supabase.from('schedules').select('id')
        .eq('store_id', emp.store_id).eq('week_start', weekKey).single()
  ...
  const ws = weekStart
  const we = addWeeks(weekStart, 0)
  const weekLabel = `${ws.getDate()} – ${addWeeks(ws, 0).getDate() + 6} ${MONTHS[ws.getMonth()]} ${ws.getFullYear()}`
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <div>
                        ...
                        <span className="text-xs text-gray-400 ml-2">{(emp as any).stores?.name}</span>
                      </div>
